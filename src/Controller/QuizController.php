...
class QuizController extends AbstractController
{
    ...
    public function quiz(User $user, QuizRepository $quizRepo, SessionInterface $session): Response
    {
        ...
        if(!$quizRepo->findByUser($user->getId())) {
            ...
            //create an assiociative array 
            $quizzes = [
                
            "Accessibilité" => [
                "accessibilité-48.png"=> [
            ...
            foreach($quizzes as $keyTheme => $name){

                //var_dump($keyTheme);
                //var_dump($keyTheme);
                //nom des themes
                $quiz = new Quiz();
                $quiz->setName($keyTheme);
                $quiz->setUser($user);
                
              

                foreach($name as $keyIcone => $title){

                    //nom des icones
                    var_dump($keyIcone);
                    $quiz->setIcone($keyIcone);
                    $em = $this->getDoctrine()->getManager();
                    $em->persist($quiz);
                    $em->flush();
               
                 
                    foreach($title as $keyQuestions =>$titlesAll){
                        //var_dump($keyQuestions);
                        $questionsq = new Questions();
                        
                        // title questions
                        $questionsq->setTitle($keyQuestions);
                        //dd($keyQuestions);
